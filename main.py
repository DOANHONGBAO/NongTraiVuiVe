...
# Hàm lưu dữ liệu người chơi vào file JSON
        ...
        try:
            ...
            if os.path.exists('players.json'):
                try:
                    ...
                    print(f"Error decoding players.json: {e}. Initializing as empty dict")
            
            
            # Update data for current player only
            players_data[current_user] = player.to_dict()
            players_data[current_user]['current_day'] = current_day
            players_data[current_user]['yard'] = []  # Placeholder
            
            # Write to a temporary file to prevent corruption
            with tempfile.NamedTemporaryFile('w', delete=False, suffix='.json') as temp_file:
                json.dump(players_data, temp_file, indent=4)
                temp_file_name = temp_file.name
            
            # Replace players.json with the temp file
    ...
    while True:
        ...
        if current_state == STATE_MENU:
            ...
            if isinstance(result, tuple):
                ...
                if action in ["go_to_farming", "back_to_menu"]:
                    save_player_data(player, current_day)
